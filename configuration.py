...
dag_config = {
    ...
    'sql_key':'''with two as (select table_name, constraint_name, array_to_string(array_agg(column_name),',') as columns, 
                                                  case 
                  ...
                  select distinct two.table_name as table_name, two.constraint_name as constraint_name, two.columns as columns,
                         two.constraint_type as constraint_type, format('%s.%s', ccu.table_schema, ccu.table_name) as foreign_table, array_to_string(array_agg(ccu.column_name), ',') as column_name
                  from two
              ...
              ''',
    'sql_create_key_tb':'''CREATE TABLE IF NOT EXISTS etl.key_table_params (table_name varchar, constraint_name varchar, columns varchar,
